...
\begin{itemize}
\item Generate around 2000 hyper-realistic customer support tickets across three difficulty tiers
...
\item Use tiered complexity (Easy: 30\%, Medium: 30\%, Hard: 40\% samples) for comprehensive analysis
\end{itemize}
...
\begin{itemize}
\item \textbf{Easy Tickets (30\%)}: Clear, unambiguous requests with high expected agreement
\item \textbf{Medium Tickets (30\%)}: Policy ambiguities testing L1/L2 boundary decisions
\item \textbf{Hard Tickets (40\%)}: Stream-of-consciousness, emotionally charged, multi-issue scenarios
\end{itemize}
...
\item \textbf{Scale Constraints}: 2000 samples sufficient for proof-of-concept but limited for production training
...
\begin{itemize}
\item \textbf{Scale Expansion}: Generate larger datasets (50000+ samples) for production model training
